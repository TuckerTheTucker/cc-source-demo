    ...
    public function new(daStage:String)
    {
        ...
        switch(daStage)
        {
            case 'halloween':
				...
				case 'dessertCity':
						camZoom = 0.8;
						curStage = 'dessertCity';
						/*
						var tempbg:FlxSprite = new FlxSprite( -600, -350).loadGraphic(Paths.image('placeholderlolol'));
						tempbg.setGraphicSize(Std.int(tempbg.width * 0.65));
						tempbg.scrollFactor.set(1, 1);
						tempbg.active = false;
						swagBacks['tempbg'] = tempbg;
                        toAdd.push(tempbg);
						*/
						
						var outside:FlxSprite = new FlxSprite(0, -150).loadGraphic(Paths.image('dstage/outside'));
						outside.scrollFactor.set(.8, .8);
						outside.setGraphicSize(Std.int(outside.width * 1.1));
						outside.screenCenter(X);
						outside.active = false;
						swagBacks['outside'] = outside;
                        toAdd.push(outside);
						
						var bg:FlxSprite = new FlxSprite(-1400, -600).loadGraphic(Paths.image('dstage/wallz'));
						bg.scrollFactor.set(1, 1);
						bg.active = false;
						swagBacks['bg'] = bg;
                        toAdd.push(bg);
						
						var tree:FlxSprite = new FlxSprite(-300, 225).loadGraphic(Paths.image('dstage/pinetree'));
						tree.scrollFactor.set(.95, 1);
						tree.active = false;
						swagBacks['tree'] = tree;
                        toAdd.push(tree);
						
						var tawog:FlxSprite = new FlxSprite(25, 350).loadGraphic(Paths.image('dstage/gumballmachine'));
						tawog.scrollFactor.set(.95, 1);
						tawog.active = false;
						swagBacks['tawog'] = tawog;
                        toAdd.push(tawog);
						
						var deskman = new FlxSprite(1350, 200);
						deskman.frames = Paths.getSparrowAtlas('dstage/manbop');
						deskman.animation.addByPrefix('idle', 'tablebop', 24, false);
						deskman.antialiasing = FlxG.save.data.antialiasing;
						deskman.scrollFactor.set(.95, 1);
						if (FlxG.save.data.distractions)
						{
						    swagBacks['deskman'] = deskman;
                            toAdd.push(deskman);
                            animatedBacks.push(deskman);
						}
				case 'dessertCityRuins':
						camZoom = 0.8;
						curStage = 'dessertCity';
						var tempbg:FlxSprite = new FlxSprite( -600, -350).loadGraphic(Paths.image('placeholderlololmelt'));
						tempbg.setGraphicSize(Std.int(tempbg.width * 0.65));
						tempbg.scrollFactor.set(1, 1);
						tempbg.active = false;
						swagBacks['tempbg'] = tempbg;
