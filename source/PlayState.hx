...
class PlayState extends MusicBeatState
{
	...
	private var unlock:FlxSound;

	public static var dad:Character;
	public static var gf:Character;
	public static var boyfriend:Boyfriend;

	//public static var theEmbiggining:Int = 0;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'fresh':
				dialogue = ["Not too shabby boy.", "but i made a nuke to blow up whoever posts this edited dialogue on twitter or reddit or any social media like that!!"];
			case 'dadbattle':
				dialogue = [
			...
			case 'snicker':
				dialogue = CoolUtil.coolTextFile(Paths.txt('snicker/demoDialogue'));
		}
		...
		for (index => array in Stage.layInFront)
		{
			switch (index)
			{
				case 0:
					add(gf);
					trace("loaded " + gfVersion);
					gf.scrollFactor.set(0.95, 0.95);
					for (bg in array)
						add(bg);
				case 1:
					add(dad);
					trace("loaded " + SONG.player2);
					for (bg in array)
						add(bg);
				case 2:
					add(boyfriend);
					trace("loaded " + SONG.player1);
					for (bg in array)
						add(bg);
			}
		}
		...
		switch (SONG.player2)
		{
			...
			case 'pepper':
				dad.x += 200;
				dad.y += 250;
				camPos.set(dad.getGraphicMidpoint().x + 50, dad.getGraphicMidpoint().y + 350);
		}
		...
		healthBar.createFilledBar(0xFFC1A461, 0xFF31B0D1);
		// healthBar :)
		add(healthBar);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		unlock = new FlxSound().loadEmbedded(Paths.sound('cheatunlock'));

		trace('loaded vocals for ' + PlayState.SONG.song.toLowerCase());

		FlxG.sound.list.add(vocals);
		FlxG.sound.list.add(unlock);
		
		trace("loaded the unlock sound. doesnt matter tho since this is a demo build");
	...
	{	
		...
		if (FlxG.keys.justPressed.TAB)
			trace("Cheat Active: " + TitleState.cheatActive);
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				if (holdArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
				{
					notes.forEachAlive(function(daNote:Note)
					{
						...
						{
							goodNoteHit(daNote); //i tried adding danote.ratings into this. it made sustains literally unusable. oh well
							health += 0.02;
						}
					});
				}
	...
	*/
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					/* //was made for shits and giggles.
					theEmbiggining += 5;
					if (dad.curCharacter == 'mom-car')
					{
						dad.setGraphicSize(Std.int(dad.width + theEmbiggining));
						dad.y -= 5;
						if (!SONG.notes[Math.floor(curStep / 16)].mustHitSection)
							camFollow.y = dad.getMidpoint().y - theEmbiggining;
							Stage.camZoom -= 0.0025;
							if (Stage.camZoom <= 0.1)
								Stage.camZoom = 0.1;
					}
					*/
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'snicker')
			camZooming = true;
